 ...
 * product name (`PULSAR`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/PULSAR'
 *
 * This is a hard failure with no obvious cause for the user — `pulsar desktop`
 ...
 * (the mac-unpacked PULSAR.app bundle) and Windows (win-unpacked), so we
