#!/usr/bin/env node
// set-exe-identity.cjs — stamp the PULSAR icon + version metadata onto the
// built PULSAR.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked PULSAR.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `pulsar desktop`, the installer's
...
// path (which rebuilds via `pulsar desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-PULSAR.exe>
//
...
// Stamp the PULSAR icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'PULSAR',
      FileDescription: 'PULSAR - Assistant IA CHU Guyane',
      CompanyName: 'CHU de Guyane',
      LegalCopyright: 'Copyright (c) 2026 CHU de Guyane'
    }
  })

  console.log('[set-exe-identity] done — PULSAR icon + identity stamped')
}
