...
const { canImportPulsarCli, verifyPulsarCli } = require('./backend-probes.cjs')
...
const USER_DATA_OVERRIDE = process.env.PULSAR_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.PULSAR_DESKTOP_DEV_SERVER
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[pulsar] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[pulsar] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[pulsar] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[pulsar] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// PULSAR_HOME — the user-facing root for everything PULSAR-related. Mirrors
// scripts/install.ps1's $PulsarHome and scripts/install.sh's $PULSAR_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\pulsar (matches install.ps1)
//   macOS / Linux: ~/.pulsar (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.pulsar directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\pulsar yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// PULSAR_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// PULSAR_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.pulsar / %LOCALAPPDATA%\pulsar.
function resolvePulsarHome() {
  if (process.env.PULSAR_HOME) return path.resolve(process.env.PULSAR_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'pulsar-home')
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'pulsar')
    const legacy = path.join(app.getPath('home'), '.pulsar')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.pulsar setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.pulsar')
}

const PULSAR_HOME = resolvePulsarHome() // PULSAR_HOME
// ACTIVE_PULSAR_ROOT — the canonical mutable PULSAR install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_PULSAR_ROOT = path.join(PULSAR_HOME, 'pulsar-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_PULSAR_ROOT, 'venv')
...
// boots, going straight to `resolvePulsarBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_PULSAR_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_PULSAR_ROOT, '.pulsar-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
// active-profile.json records which PULSAR profile the desktop launches its
// local backend as. When set, startPULSAR() passes `pulsar --profile <name>
// dashboard …`, which deterministically pins PULSAR_HOME (see
// _apply_profile_override in pulsar_cli/main.py) and bypasses the sticky
// ~/.pulsar/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors pulsar_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// pulsarDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under PULSAR_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by pulsar_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(PULSAR_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (pulsar_logging.py RotatingFileHandler, maxBytes x
...
// Seed the native About panel with the live PULSAR version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `pulsar update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolvePULSARVersion(),
...
const MEDIA_PROTOCOL = 'pulsar-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(pulsar-media://…)`.
...
let mainWindow = null
let pulsarProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by pulsarProcess +
// connectionPromise + startPULSAR(); this pool only holds EXTRA profile
...
// onto the error so subsequent startPULSAR() calls (e.g. the renderer's
...
function rememberLog(chunk) {
  ...
  if (!text) return
  const lines = text.split(/\r?\n/).map(line => `[pulsar] ${line}`)
  pulsarLog.push(...lines)
  if (pulsarLog.length > 300) {
    pulsarLog.splice(0, pulsarLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-pulsar-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('pulsar:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the pulsar:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('pulsar:bootstrap:event', ev)
}
...
function isPulsarSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'pulsar_cli', 'main.py'))
}
...
function findSystemPython() {
  ...
  // CPython but several PULSAR deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. PULSAR' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\pulsar\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'pulsar', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'pulsar', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\pulsar\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'pulsar', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'pulsar', 'git', 'bin', 'git.exe'))
  }
...
function recentPulsarLog() {
  return pulsarLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's pulsar root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_PULSAR_ROOT.
...
function resolveUpdateRoot() {
  const candidates = [
    ...
    !IS_PACKAGED && isPulsarSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isPulsarSourceRoot(ACTIVE_PULSAR_ROOT) ? ACTIVE_PULSAR_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_PULSAR_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('pulsar:updates:progress', merged)
  }
}
...
// PULSAR_HOME/pulsar-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_pulsar_home). That binary owns
// ALL repo mutation — running `pulsar update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'pulsar-setup.exe' : 'pulsar-setup'
  const candidate = path.join(PULSAR_HOME, name)
...
// Path to the venv shim whose lock decides whether `pulsar update` can write
// fresh entry points. On Windows this is the file the running backend
// `pulsar.exe` holds open; on POSIX it's never mandatory-locked.
function venvPulsarShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'pulsar.exe')
    : path.join(updateRoot, 'venv', 'bin', 'pulsar')
}
...
// only signals the direct child, so on Windows a backend `pulsar.exe` that
// spawned its own grandchildren (a `pulsar` REPL, a pty terminal session, the
...
// `pulsarProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `pulsar.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a pulsar
// REPL, a pty terminal, the gateway) keep `pulsar.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (pulsarProcess && Number.isInteger(pulsarProcess.pid)) pids.push(pulsarProcess.pid)
  ...
  if (pulsarProcess && !pulsarProcess.killed) {
    try {
      pulsarProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvPulsarShimPath(updateRoot)
...
// `pulsar update`). Instead we spawn the staged PULSAR-Setup binary with
// --update and quit, so it can run `pulsar update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri pulsar-setup. Unlike Windows
      ...
      // whole update itself: `pulsar update` (backend) + `pulsar desktop
    ...
    if (!updater) {
      ...
      // `pulsar desktop`, never the Tauri installer that self-copies
      // pulsar-setup.exe into PULSAR_HOME). They DO have a working `pulsar`
      ...
      // checkout they're on — bare `pulsar update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'pulsar update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `pulsar update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `pulsar update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, pulsarRoot: updateRoot }
    }

    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the PULSAR updater…', percent: 100 })
    ...
    // pulsar.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `pulsar update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: PULSAR_HOME,
      env: {
        ...process.env,
        PULSAR_HOME,
        PATH: [path.join(PULSAR_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
// Resolve the pulsar CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `pulsar` on PATH.
function resolvePulsarCliBinary(updateRoot) {
  const venvPulsar = path.join(updateRoot, 'venv', 'bin', 'pulsar')
  if (fileExists(venvPulsar)) return venvPulsar
  return findOnPath('pulsar') || null
}
...
// macOS/Linux in-app update: backend (`pulsar update`) + OS-aware GUI rebuild
// (`pulsar desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const pulsar = resolvePulsarCliBinary(updateRoot)
  if (!pulsar) {
    emitUpdateProgress({ stage: 'manual', message: 'pulsar update', percent: null })
    return { ok: true, manual: true, command: 'pulsar update', pulsarRoot: updateRoot }
  }

  // Put the PULSAR-managed Node and the venv on PATH so `pulsar desktop`'s
  // npm build can find them on a machine with no system Node.
  const extraPath = [path.join(PULSAR_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    PULSAR_HOME,
  ...
  // `pulsar update` reaps stale `pulsar dashboard` backends (a code update
  ...
  if (pulsarProcess && Number.isInteger(pulsarProcess.pid)) {
    desktopChildPids.push(pulsarProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating PULSAR (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(pulsar, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'pulsar update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'pulsar update failed' }
  }

  emitUpdateProgress({ stage: 'rebuild', message: 'Rebuilding the desktop app…', percent: 60 })
  const rebuilt = await runStreamedUpdate(pulsar, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart PULSAR to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'PULSAR.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'PULSAR.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart PULSAR to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.pulsar-update-new"; then
    rm -rf "$DST.pulsar-update-old" 2>/dev/null || true
    mv "$DST" "$DST.pulsar-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.pulsar-update-new" "$DST"
    rm -rf "$DST.pulsar-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `pulsar-desktop-update-${Date.now()}.sh`)
  try {
    fs.writeFileSync(scriptPath, swapScript, { mode: 0o755 })
  } catch (err) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart PULSAR to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `pulsar
  ...
  return isPulsarSourceRoot(ACTIVE_PULSAR_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: pulsar desktop --force-build`
  )
  return candidates[0]
}
...
function resolvePULSARCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/PULSAR.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolvePULSARCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'pulsar_cli.main', ...dashboardArgs],
    env: {
...
// createActiveBackend — build a backend pointing at ACTIVE_PULSAR_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `PULSAR at ${ACTIVE_PULSAR_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'pulsar_cli.main', ...dashboardArgs],
    env: {
      PYTHONPATH: [ACTIVE_PULSAR_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
    },
    root: ACTIVE_PULSAR_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolvePulsarBackend(dashboardArgs) {
  // 1. Explicit override -- HERMES_DESKTOP_HERMES_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.HERMES_DESKTOP_HERMES_ROOT && path.resolve(process.env.HERMES_DESKTOP_HERMES_ROOT)
  if (overrideRoot && isPulsarSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `PULSAR source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `pulsar` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isPulsarSourceRoot.)
  if (!IS_PACKAGED && isPulsarSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `PULSAR source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_PULSAR_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\pulsar\pulsar-agent (Windows) or ~/.pulsar/pulsar-agent.
  ...
  //    to spawning pulsar. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `pulsar update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `pulsar` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    HERMES_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    let pulsarCommand = null
    const pulsarOverride = process.env.HERMES_DESKTOP_HERMES

    if (pulsarOverride) {
      const resolvedOverride = findOnPath(pulsarOverride)
      if (resolvedOverride) {
        pulsarCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(pulsarOverride, { isWsl: IS_WSL })) {
        pulsarCommand = pulsarOverride
      } else {
        rememberLog(`Ignoring Windows PULSAR override under WSL: ${pulsarOverride}`)
      }
    } else {
      pulsarCommand = findOnPath('pulsar')
    }

    if (pulsarCommand) {
      if (looksLikeDesktopAppBinary(pulsarCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving PULSAR CLI: ${pulsarCommand}`)
        pulsarCommand = null
      }
    }

    if (pulsarCommand) {
      // Smoke-test the candidate before trusting it. A `pulsar` shim
      ...
      const shellForProbe = isCommandScript(pulsarCommand)
      if (verifyPulsarCli(pulsarCommand, { shell: shellForProbe })) {
        return {
          label: `existing PULSAR CLI at ${pulsarCommand}`,
      ...
      rememberLog(
        `Ignoring existing PULSAR CLI at ${pulsarCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed pulsar_cli module via system Python.
  ...
  if (python) {
    ...
    // having pulsar_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\pulsar\pulsar-agent\venv.
    if (canImportPulsarCli(python)) {
      return {
        kind: 'python',
        label: `installed pulsar_cli module via ${python}`,
        command: python,
        args: ['-m', 'pulsar_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: pulsar_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolvePulsarBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'PULSAR Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolvePulsarBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no PULSAR install found; starting first-launch bootstrap')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      pulsarHome: PULSAR_HOME,
      logRoot: path.join(PULSAR_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('PULSAR install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `PULSAR bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(PULSAR_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startPULSAR() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // pulsar:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolvePulsarBackend(backend.args))
  }
  ...
  if (!isPulsarSourceRoot(ACTIVE_PULSAR_ROOT)) {
    throw new Error(
      `PULSAR install at ${ACTIVE_PULSAR_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. PULSAR' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\pulsar\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `pulsar` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for PULSAR on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch PULSAR.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isPulsarSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `PULSAR venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `PULSAR at ${ACTIVE_PULSAR_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'PULSAR runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported PULSAR backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the PULSAR backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to PULSAR backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-PULSAR-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('pulsar:link-titles', { cache: false })
...
function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolvePULSARCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('pulsar:preview-file-changed', payload)
}
...
async function waitForPULSAR(baseUrl, token) {
  ...
  throw new Error(`PULSAR backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('pulsar:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('pulsar:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting PULSAR...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('pulsar:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('pulsar:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('pulsar:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'pulsar:desktop:zoomLevel'
...
//
// Hosted PULSAR gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``pulsar_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / pulsar #37247).
//     Both are set as HttpOnly cookies (``pulsar_session_at`` ~15 min,
//     ``pulsar_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:pulsar-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to PULSAR gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to PULSAR backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root PULSAR_HOME explicitly); null means "no
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (pulsar #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote PULSAR gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote PULSAR gateway is selected, but no session token is saved. ' +
    ...
    if (!rawEnvToken) {
      throw new Error(
        'HERMES_DESKTOP_REMOTE_URL is set but HERMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote PULSAR backend.'
      )
    }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every PULSAR
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startPULSAR())
  ...
  // connect to PULSAR gateway". Mirror the renderer's connect here so the test
...
function resetPULSARConnection() {
...
// startPULSAR() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetPULSARConnection() nulls pulsarProcess.
  const dying = pulsarProcess && !pulsarProcess.killed ? pulsarProcess : null
  resetPULSARConnection()
...
// profile to startPULSAR() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startPULSAR() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForPULSAR(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: pulsarLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  // --profile wins over the inherited PULSAR_HOME env (see _apply_profile_override
  // step 3 in pulsar_cli/main.py), so the child re-homes to this profile.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', String(port)]
  const backend = await ensureRuntime(resolvePulsarBackend(dashboardArgs))
  const pulsarCwd = resolvePULSARCwd()
  const webDist = resolveWebDist()

  rememberLog(`Starting PULSAR backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`PULSAR backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`PULSAR backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`PULSAR backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })

  const baseUrl = `http://127.0.0.1:${port}`
  await Promise.race([waitForPULSAR(baseUrl, token), startFailed])
  ready = true
...
async function startPULSAR() {
  ...
  // process, every subsequent startPULSAR() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving PULSAR backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote PULSAR backend at ${remote.baseUrl}`, 24)
      await waitForPULSAR(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote PULSAR backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.pulsar/active_profile file) and
    // resolves PULSAR_HOME the same way `pulsar -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving PULSAR runtime', 28)
    const backend = await ensureRuntime(resolvePulsarBackend(dashboardArgs))
    const pulsarCwd = resolvePULSARCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting PULSAR backend via ${backend.label}`, 84)
    rememberLog(`Starting PULSAR backend via ${backend.label}`)

    pulsarProcess = spawn(backend.command, backend.args, {
      cwd: pulsarCwd,
      env: {
        ...process.env,
        // Explicitly pin PULSAR_HOME for the child so Python's get_pulsar_home()
        // resolves to the SAME location our resolvePulsarHome() picked. Without
        // this pin, Python falls back to ~/.pulsar on every platform — fine on
        // mac/linux (where our default matches), but on Windows our default is
        // %LOCALAPPDATA%\pulsar, which differs from C:\Users\<u>\.pulsar.
        // Mismatch would split config / sessions / .env / logs across two
        // directories. install.ps1 sets PULSAR_HOME via setx; the desktop
    ...
    pulsarProcess.stdout.on('data', rememberLog)
    pulsarProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    pulsarProcess.once('error', error => {
      rememberLog(`PULSAR backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `PULSAR backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    pulsarProcess.once('exit', (code, signal) => {
      rememberLog(`PULSAR backend exited (${signal || code})`)
      pulsarProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `PULSAR backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `PULSAR backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentPulsarLog()}`
          )
        )
      }
    })
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for PULSAR backend to become ready', 90)
    await Promise.race([waitForPULSAR(baseUrl, token), backendStartFailed])
    backendReady = true
    updateBootProgress({
      phase: 'backend.ready',
      message: 'PULSAR backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startPULSAR().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('pulsar:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting PULSAR…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('pulsar:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetPULSARConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote PULSAR backend failed liveness probe; dropping stale connection.')
    resetPULSARConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('pulsar:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('pulsar:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('pulsar:window:openSession', async (_event, sessionId) => {
...
ipcMain.handle('pulsar:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startPULSAR() call restarts the
...
ipcMain.handle('pulsar:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startPULSAR() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('pulsar:bootstrap:cancel', async () => {
...
ipcMain.handle('pulsar:boot-progress:get', async () => bootProgressState)
ipcMain.handle('pulsar:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('pulsar:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('pulsar:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('pulsar:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('pulsar:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('pulsar:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('pulsar:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('pulsar:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('pulsar:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('pulsar:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new PULSAR_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('pulsar:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('pulsar:requestMicrophoneAccess', async () => {
...
ipcMain.handle('pulsar:api', async (_event, request) => {
...
ipcMain.handle('pulsar:notify', (_event, payload) => {
  if (!Notification.isSupported()) return false
  new Notification({
    title: payload?.title || 'PULSAR',
    body: payload?.body || '',
...
ipcMain.handle('pulsar:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('pulsar:readFileText', async (_event, filePath) => {
...
ipcMain.handle('pulsar:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('pulsar:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('pulsar:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('pulsar:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('pulsar:saveClipboardImage', async () => {
...
ipcMain.handle('pulsar:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('pulsar:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('pulsar:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('pulsar:titlebar-theme', (_event, payload) => {
...
ipcMain.handle('pulsar:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolvePULSARCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('pulsar:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolvePULSARCwd()
}))

ipcMain.handle('pulsar:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('pulsar:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('pulsar:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('pulsar:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('pulsar:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('pulsar:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: pulsarLog.slice(-200) }))
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip PULSAR' TUI into
  ...
  // Let a pulsar/--tui launched in this pane know it's embedded in the desktop
...
function terminalChannel(id, suffix) {
  return `pulsar:terminal:${id}:${suffix}`
}
...
ipcMain.handle('pulsar:fs:readDir', async (_event, dirPath) => {
...
ipcMain.handle('pulsar:fs:gitRoot', async (_event, startPath) => {
...
ipcMain.handle('pulsar:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart PULSAR.')
  }
...
ipcMain.handle('pulsar:terminal:write', (_event, id, data) => {
...
ipcMain.handle('pulsar:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('pulsar:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('pulsar:updates:check', async () =>
...
ipcMain.handle('pulsar:updates:apply', async (_event, payload) =>
...
ipcMain.handle('pulsar:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('pulsar:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical PULSAR version (the one `release.py` bumps in
// pulsar_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real PULSAR version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolvePULSARVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'pulsar_cli', '__init__.py')
...
// Re-resolve the live PULSAR version and push it into the native About panel
// just before showing it, so an in-place `pulsar update` is reflected without
...
ipcMain.handle('pulsar:version', async () => ({
  appVersion: resolvePULSARVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  pulsarRoot: resolveUpdateRoot()
}))
...
// the actual removal via `pulsar uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (pulsar_cli/uninstall.py + pulsar_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_PULSAR_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isPulsarSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(py, ['-m', 'pulsar_cli.main', 'uninstall', '--gui-summary'], {
        cwd: agentRoot,
        env: { ...process.env, PULSAR_HOME, NO_COLOR: '1' },
        stdio: ['ignore', 'pipe', 'ignore']
      })
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no PULSAR agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import pulsar_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_PULSAR_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `pulsar-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `pulsar-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('pulsar:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('pulsar:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('pulsar:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('pulsar:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))
