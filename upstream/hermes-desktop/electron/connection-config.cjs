 ...
 *     `X-PULSAR-Session-Token` header; WS uses `?token=`.
...
// pulsar_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - pulsar_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - pulsar_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / pulsar #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-pulsar_session_at', '__Secure-pulsar_session_at', 'pulsar_session_at']
const RT_COOKIE_VARIANTS = ['__Host-pulsar_session_rt', '__Secure-pulsar_session_rt', 'pulsar_session_rt']
...
/**
 * True if any cookie in `cookies` is a pulsar session ACCESS-token cookie
