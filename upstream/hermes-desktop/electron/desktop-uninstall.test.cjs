...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'pulsar_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'pulsar_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'pulsar_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/PULSAR.app/Contents/MacOS/PULSAR', 'darwin'),
    '/Applications/PULSAR.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/PULSAR.app/Contents/MacOS/PULSAR', 'darwin'),
    '/Users/x/Applications/PULSAR.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\PULSAR\\PULSAR.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\PULSAR'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\pulsar-desktop\\PULSAR.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\pulsar-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\PULSAR.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_PULSARXXXX/pulsar', 'linux', { APPIMAGE: '/home/x/Apps/PULSAR.AppImage' }),
    '/home/x/Apps/PULSAR.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(
    resolveRemovableAppPath('/opt/pulsar/linux-unpacked/pulsar', 'linux', {}),
    '/opt/pulsar/linux-unpacked'
  )
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/pulsar', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/PULSAR.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/PULSAR.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.pulsar/pulsar-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.pulsar/pulsar-agent',
    uninstallArgs: ['-m', 'pulsar_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/pulsar/linux-unpacked',
    pulsarHome: '/home/x/.pulsar'
  })
  ...
  assert.match(script, /'-m' 'pulsar_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/pulsar\/linux-unpacked'/)
  assert.match(script, /export HERMES_HOME='\/home\/x\/\.pulsar'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.pulsar/pulsar-agent',
    agentRoot: '/home/x/.pulsar/pulsar-agent',
    uninstallArgs: ['-m', 'pulsar_cli.uninstall', '--mode', 'full'],
    appPath: null,
    pulsarHome: '/home/x/.pulsar'
  })
  // System python + source on PYTHONPATH so import pulsar_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.pulsar\/pulsar-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'pulsar_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'pulsar_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    pulsarHome: '/h'
  })
  assert.doesNotMatch(script, /rm -rf '\//)
  // Still runs the uninstall.
  assert.match(script, /'-m' 'pulsar_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\pulsar',
    uninstallArgs: ['-m', 'pulsar_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\PULSAR',
    pulsarHome: 'C:\\Users\\x\\AppData\\Local\\pulsar'
  })
  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import pulsar_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\pulsar;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "pulsar_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\PULSAR" >nul 2>&1/)
