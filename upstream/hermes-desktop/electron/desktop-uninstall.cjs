 ...
 * the three user-facing uninstall modes to the `pulsar uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m pulsar_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `pulsar_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'pulsar_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/PULSAR.app/Contents/MacOS/PULSAR  → …/PULSAR.app
 *   Windows: …\PULSAR\PULSAR.exe                 → …\PULSAR  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/PULSAR.app/Contents/MacOS/PULSAR → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/PULSAR.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs PULSAR.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]PULSAR$/i.test(dir) || /[\\/]pulsar-desktop$/i.test(dir)) return dir
    return null
  }

  // Linux: an AppImage exposes its own path via the APPIMAGE env var.
  if (env.APPIMAGE) return env.APPIMAGE
  // Unpacked electron-builder tree: …/linux-unpacked/pulsar
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import pulsar_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, pulsarHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export PULSAR_HOME=${q(pulsarHome)}`
  ]
  if (pythonPath) {
 ...
 * modes + `pythonPath`=agentRoot so `import pulsar_cli` resolves from source
 ...
 */
function buildWindowsCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, pulsarHome }) {
  const pid = Number(desktopPid) || 0
  // cmd.exe has no string escaping inside quotes; strip embedded quotes (paths
  // under %LOCALAPPDATA% never contain them). `&`/`^` in a path would still be
  // a problem, but PULSAR install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "PULSAR_HOME=${String(pulsarHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
  if (pythonPath) {
