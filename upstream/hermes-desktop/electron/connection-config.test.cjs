...
test('profileRemoteOverride returns the per-profile remote with defaulted auth mode', () => {
  const config = {
    profiles: {
      coder: { mode: 'remote', url: '  https://coder.example.com/pulsar  ', token: { value: 'sek' } }
    }
  }
  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://coder.example.com/pulsar',
    authMode: 'token',
    token: { value: 'sek' }
  })
})
...
test('normalizeRemoteBaseUrl strips trailing slashes, hash, and query', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/pulsar/'), 'https://gw.example.com/pulsar')
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/pulsar?x=1#frag'), 'https://gw.example.com/pulsar')
})

test('normalizeRemoteBaseUrl preserves a path prefix', () => {
  assert.equal(normalizeRemoteBaseUrl('https://host/pulsar'), 'https://host/pulsar')
})
...
test('buildGatewayWsUrl honors a path prefix', () => {
  assert.equal(buildGatewayWsUrl('https://host/pulsar', 't'), 'wss://host/pulsar/api/ws?token=t')
})
...
test('buildGatewayWsUrlWithTicket uses ?ticket= not ?token=', () => {
  const url = buildGatewayWsUrlWithTicket('https://gw.example.com/pulsar', 'tkt-9')
  assert.equal(url, 'wss://gw.example.com/pulsar/api/ws?ticket=tkt-9')
...
test('cookiesHaveSession detects the bare access-token cookie', () => {
  assert.equal(cookiesHaveSession([{ name: 'pulsar_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession detects the __Host- and __Secure- prefixed variants', () => {
  assert.equal(cookiesHaveSession([{ name: '__Host-pulsar_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveSession([{ name: '__Secure-pulsar_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession is false for an empty value', () => {
  assert.equal(cookiesHaveSession([{ name: 'pulsar_session_at', value: '' }]), false)
})
...
test('cookiesHaveSession ignores unrelated cookies (AT-only by design)', () => {
  ...
  assert.equal(cookiesHaveSession([{ name: 'pulsar_session_rt', value: 'x' }]), false)
...
test('AT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(AT_COOKIE_VARIANTS, ['__Host-pulsar_session_at', '__Secure-pulsar_session_at', 'pulsar_session_at'])
})

test('RT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(RT_COOKIE_VARIANTS, ['__Host-pulsar_session_rt', '__Secure-pulsar_session_rt', 'pulsar_session_rt'])
})

// --- cookiesHaveLiveSession (AT or RT — the connectivity check) ---

test('cookiesHaveLiveSession is true for a live access-token cookie', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'pulsar_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-pulsar_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-pulsar_session_at', value: 'x' }]), true)
})
...
test('cookiesHaveLiveSession is true for an RT cookie even with NO access-token cookie', () => {
  ...
  assert.equal(cookiesHaveLiveSession([{ name: 'pulsar_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-pulsar_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-pulsar_session_rt', value: 'x' }]), true)
})

test('cookiesHaveLiveSession is true when both AT and RT are present', () => {
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'pulsar_session_at', value: 'a' },
      { name: 'pulsar_session_rt', value: 'r' }
    ]),
    true
  )
})
...
test('cookiesHaveLiveSession is false for empty values', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'pulsar_session_at', value: '' }]), false)
  assert.equal(cookiesHaveLiveSession([{ name: 'pulsar_session_rt', value: '' }]), false)
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'pulsar_session_at', value: '' },
      { name: 'pulsar_session_rt', value: '' }
    ]),
    false
  )
})
