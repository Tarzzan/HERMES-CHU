...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'pulsar-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/pulsar-runner-test',
    sourceRepoRoot: null,
    pulsarHome: '/tmp/pulsar-runner-test',
    logRoot: '/tmp/pulsar-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'pulsar-agent', 'scripts')
