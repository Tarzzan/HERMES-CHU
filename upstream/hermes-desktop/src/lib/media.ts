...
export function mediaStreamUrl(path: string): string {
  return `pulsar-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// expose GET /api/media (pulsar_cli/web_server.py).
export async function gatewayMediaDataUrl(path: string): Promise<string> {
  const file = filePathFromMediaPath(path)

  const result = await window.pulsarDesktop!.api<{ data_url: string }>({
