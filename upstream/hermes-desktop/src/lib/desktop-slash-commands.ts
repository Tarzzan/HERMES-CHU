...
const DESKTOP_COMMAND_META = [
  ...
  ['/profile', 'Switch the active PULSAR profile'],
  ...
  ['/version', 'Show PULSAR Agent version'],
...
export function isDesktopSlashCommand(command: string): boolean {
  ...
  return DESKTOP_COMMANDS.has(canonical) || !isKnownPULSARSlashCommand(normalized)
}

/**
 * An "extension" command is anything the backend surfaces that is NOT one of
 * PULSAR' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownPULSARSlashCommand(normalized)
}
...
function isKnownPULSARSlashCommand(command: string): boolean {
