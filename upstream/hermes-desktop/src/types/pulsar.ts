...
export type PULSARConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `pulsar model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
export interface ToolProvider {
   ...
   *  CLI `pulsar tools` active-provider detection). */
  is_active: boolean
}
...
/** Shape of `GET /api/pulsar/update/check` — the backend's own update state.
