import type { ConnectionState, GatewayEvent } from '@pulsar/shared'
...
const isOpen = (gateway: PulsarGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<PulsarGateway | null>(null)
...
let primaryGateway: PulsarGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: PulsarGateway | null, profile = 'default'): void {
...
export function activeGateway(): PulsarGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<PulsarGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
