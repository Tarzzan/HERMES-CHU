...
const checkPULSARUpdateSpy = vi.fn()
const updatePULSARSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/pulsar', () => ({
  checkPULSARUpdate: (...args: unknown[]) => checkPULSARUpdateSpy(...args),
  updatePULSAR: (...args: unknown[]) => updatePULSARSpy(...args),
...
describe('checkBackendUpdates', () => {
  beforeEach(() => {
    storage.clear()
    notifySpy.mockClear()
    checkPULSARUpdateSpy.mockReset()
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkPULSARUpdateSpy.mockResolvedValue({
      install_method: 'git',
      current_version: '0.16.0',
      behind: 2,
      update_available: true,
      can_apply: true,
      update_command: 'pulsar update',
      message: null,
    ...
    expect(checkPULSARUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkPULSARUpdateSpy).not.toHaveBeenCalled()
  })
})
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    updatePULSARSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkPULSARUpdateSpy.mockResolvedValue({ install_method: 'git', current_version: '0.16.0', behind: 0, update_available: false, can_apply: true, update_command: 'pulsar update', message: null })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updatePULSARSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkPULSARUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
