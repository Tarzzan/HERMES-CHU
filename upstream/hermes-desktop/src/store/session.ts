...
import type { SessionInfo, UsageStats } from '@/types/pulsar'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'pulsar.desktop.workspace-cwd'
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.pulsarDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.pulsarDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<PULSARConnection | null>(null)
...
export const setConnection = (next: Updater<PULSARConnection | null>) => updateAtom($connection, next)
