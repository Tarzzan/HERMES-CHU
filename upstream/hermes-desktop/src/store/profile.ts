...
import { getProfiles, setApiRequestProfile } from '@/pulsar'
...
import type { ProfileInfo } from '@/types/pulsar'
...
// /api/profiles/active `current`). "default" is the root ~/.pulsar. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'pulsar.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'pulsar.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.pulsarDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.pulsarDesktop.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'pulsar.desktop.showAllProfiles'
...
// Switch to the default (root ~/.pulsar) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.pulsarDesktop?.touchBackend?.(target).catch(() => undefined)
}
