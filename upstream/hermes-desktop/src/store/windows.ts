...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.pulsarDesktop?.openSessionWindow === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string): Promise<void> {
  ...
  try {
    const result = await window.pulsarDesktop.openSessionWindow(sessionId)
