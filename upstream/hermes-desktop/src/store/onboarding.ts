...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/pulsar'
...
const CONFIGURED_CACHE_KEY = 'pulsar-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'pulsar-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'PULSAR is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Nous Tool Gateway,
// mirroring pulsar_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
async function fetchProviderDefaultModel(
  ...
  // `pulsar model` does (for Nous it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but PULSAR still cannot resolve a usable provider. ${detail}`
    : 'Connected, but PULSAR still cannot resolve a usable provider.'
}
...
async function openSignInUrl(url: string) {
  if (window.pulsarDesktop?.openExternal) {
    try {
      await window.pulsarDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `PULSAR still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but PULSAR still cannot reach ${url}.` }
    }
