...
import { checkPULSARUpdate, getActionStatus, updatePULSAR } from '@/pulsar'
import { translateNow } from '@/i18n'
import { persistString, storedString } from '@/lib/storage'
import { dismissNotification, notify } from '@/store/notifications'
import { $connection } from '@/store/session'
import type { BackendUpdateCheckResponse } from '@/types/pulsar'
...
const UPDATE_TOAST_SNOOZE_KEY = 'pulsar:update-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: { label: translateNow('notifications.updatePULSAR'), onClick: () => void applyBackendUpdate() },
    durationMs: 0,
    id: SKEW_TOAST_ID,
    kind: 'warning',
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.pulsarDesktop?.getVersion?.()
...
export async function checkBackendUpdates(): Promise<DesktopUpdateStatus | null> {
  ...
  try {
    const status = mapBackendCheck(await checkPULSARUpdate(true))
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  const bridge = window.pulsarDesktop?.updates
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `pulsar update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'pulsar update',
        command: result.command ?? 'pulsar update'
      })
    }
...
export async function applyBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    const started = await updatePULSAR()

    if (!started.ok) {
      const message = (started as { message?: string }).message || translateNow('updates.applyStatus.notAvailable')
      const command = (started as { update_command?: string }).update_command || 'pulsar update'
