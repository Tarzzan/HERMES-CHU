  ...
  const completeAssistantMessage = useCallback(
    (sessionId: string, text: string) => {
      ...
      if (document.hidden && sessionId === activeSessionIdRef.current) {
        void window.pulsarDesktop?.notify({
          title: 'PULSAR finished',
  ...
  const failAssistantMessage = useCallback(
    (sessionId: string, errorMessage: string) => {
      updateSessionState(sessionId, state => {
        ...
        const groupId = state.pendingBranchGroup ?? undefined
        const prev = state.messages
        const error = errorMessage.trim() || 'PULSAR reported an error'
  ...
  const handleGatewayEvent = useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        const errorMessage = payload?.message || 'PULSAR reported an error'
