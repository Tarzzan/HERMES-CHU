...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<PulsarGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.pulsarDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<PulsarGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as PulsarGateway | null
      }),
    []
  )
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}) => {
      ...
      if (!gateway) {
        throw new Error('PULSAR gateway unavailable')
      }
