...
// bug that drives the REAL useGatewayBoot hook + REAL PulsarGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshPULSARConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { pulsarDesktop?: unknown }).pulsarDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { pulsarDesktop?: unknown }).pulsarDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForPULSAR) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startPULSAR()'s remote branch awaits waitForPULSAR() for 45s before it
    ...
    ;(window as { pulsarDesktop?: unknown }).pulsarDesktop = desktop
    ...
    // After ~45s waitForPULSAR gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('PULSAR backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
