...
import type { PULSARConnection } from '@/global'
import { PulsarGateway } from '@/pulsar'
...
import type { RpcEvent } from '@/types/pulsar'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.pulsarDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: PulsarGateway | null) => void
  refreshPULSARConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.pulsarDesktop

    const publish = (next: PULSARConnection | null) => {
    ...
    // composer stayed disabled forever on "Démarrage PULSAR...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Démarrage PULSAR…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to PULSAR gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshPULSARConfig().catch(() => undefined)
    ...
    const gateway = new PulsarGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshPULSARConfig()
