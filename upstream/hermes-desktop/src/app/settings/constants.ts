...
export const EMPTY_SELECT_VALUE = '__pulsar_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted PULSAR & Nous-trained models',
    docsUrl: 'https://github.com/Tarzzan/PULSAR-CHU',
    priority: 0
  },
  {
    prefix: 'OPENROUTER_',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // pulsar_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when PULSAR needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before PULSAR stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters PULSAR can read from one file request.',
  approvals: {
    mode: 'How PULSAR handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When PULSAR updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
