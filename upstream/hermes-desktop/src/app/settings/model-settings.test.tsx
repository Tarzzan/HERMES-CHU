...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nous', model: 'pulsar-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      { name: 'Nous', slug: 'nous', models: ['pulsar-4', 'pulsar-4-mini'], authenticated: true },
  ...
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nous', model: 'pulsar-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  setModelAssignment.mockResolvedValue({ provider: 'nous', model: 'pulsar-4', gateway_tools: [] })
...
describe('ModelSettings', () => {
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nous', model: 'pulsar-4' }]
    })
