...
import {
  getElevenLabsVoices,
  getPULSARConfigDefaults,
  getPULSARConfigRecord,
  getPULSARConfigSchema,
  savePULSARConfig
} from '@/pulsar'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, PULSARConfigRecord } from '@/types/pulsar'
...
export function ConfigSettings({
  ...
  const { t } = useI18n()
  const c = t.settings.config
  const [config, setConfig] = useState<PULSARConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<PULSARConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getPULSARConfigRecord(), getPULSARConfigDefaults(), getPULSARConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: PULSARConfigRecord) => {
