...
import type { ActionStatusResponse, AnalyticsResponse, StatusResponse } from '@/pulsar'
...
export function CommandCenterView({ initialSection, onClose, onDeleteSession, onOpenSession }: CommandCenterViewProps) {
  ...
  const runSystemAction = useCallback(
    async (kind: 'restart' | 'update') => {
      ...
      try {
        const started = kind === 'restart' ? await restartGateway() : await updatePULSAR()
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                          {cc.pulsarActiveSessions(status.version, status.active_sessions)}
                        </div>
                      </div>
