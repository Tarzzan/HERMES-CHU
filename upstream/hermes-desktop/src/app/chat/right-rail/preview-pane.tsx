        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.pulsarDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask PULSAR to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      target.kind !== 'file' ||
      !window.pulsarDesktop?.watchPreviewFile ||
      !window.pulsarDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.pulsarDesktop.onPreviewFileChanged(payload => {
    ...
    void window.pulsarDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.pulsarDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.pulsarDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:pulsar-preview')
