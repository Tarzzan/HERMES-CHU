...
import { PULSAR_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  if (window.pulsarDesktop.readFileText) {
    try {
      return await window.pulsarDesktop.readFileText(filePath)
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)

      if (!message.includes("No handler registered for 'pulsar:readFileText'")) {
        throw error
      }
    }
  }
  ...
  const dataUrl = await window.pulsarDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(PULSAR_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
    ...
    async function load() {
      ...
      try {
        if (isImage) {
          ...
          const dataUrl = target.dataUrl || (await window.pulsarDesktop.readFileDataUrl(filePath))
