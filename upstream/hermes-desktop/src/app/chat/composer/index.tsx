...
import { pulsarDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
import { extractDroppedFiles, PULSAR_PATHS_MIME, partitionDroppedFiles } from '../hooks/use-composer-actions'
...
export function ChatBar({
  ...
  // cold start ("Démarrage PULSAR...") from a dropped connection we're trying to
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, PULSAR_PATHS_MIME)) {
      return
    }
