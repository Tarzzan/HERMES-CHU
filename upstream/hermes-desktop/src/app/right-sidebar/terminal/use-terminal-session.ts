...
const PULSAR_PATHS_MIME = 'application/x-pulsar-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(PULSAR_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(PULSAR_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const terminalApi = window.pulsarDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      fontSize: 11,
      lineHeight: 1.12,
      // Full-screen TUIs (pulsar --tui, vim) grab the mouse, so a plain drag
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[pulsar-terminal] WebGL unavailable; falling back to DOM', err)
      }
