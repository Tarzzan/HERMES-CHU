...
import type { PULSARReadDirEntry, PULSARReadDirResult } from '@/global'

export type ProjectTreeEntry = PULSARReadDirEntry
...
async function gitRootFor(start: string) {
  if (!window.pulsarDesktop?.gitRoot) {
    return null
  }
  ...
  if (!cached) {
    cached = window.pulsarDesktop.gitRoot(key)
...
async function readGitignore(dir: string): Promise<GitignoreRule | null> {
  if (!window.pulsarDesktop?.readDir || !window.pulsarDesktop.readFileDataUrl) {
    return null
  }

  try {
    const listing = await window.pulsarDesktop.readDir(dir)

    if (!listing.entries.some(e => e.name === '.gitignore' && !e.isDirectory)) {
      return null
    }

    const text = decodeDataUrl(await window.pulsarDesktop.readFileDataUrl(`${dir}/.gitignore`))
...
function ignoredBy(rules: GitignoreRule[], entry: PULSARReadDirEntry) {
...
async function filterIgnored(entries: PULSARReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<PULSARReadDirResult> {
  if (!window.pulsarDesktop) {
    return { entries: [], error: 'no-bridge' }
  }

  const result = await window.pulsarDesktop.readDir(dirPath)
