...
import { getSessionMessages, listSessions } from '@/pulsar'
...
import type { SessionInfo, SessionMessage } from '@/types/pulsar'
  ...
  const openArtifact = useCallback(async (href: string) => {
    try {
      if (window.pulsarDesktop?.openExternal) {
        await window.pulsarDesktop.openExternal(href)
      } else {
