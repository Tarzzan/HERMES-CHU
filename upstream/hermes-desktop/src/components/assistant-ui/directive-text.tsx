...
type PULSARRefType = (typeof HERMES_REF_TYPES)[number]
...
const ICON_PATHS: Record<PULSARRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as PULSARRefType] ?? ICON_FALLBACK
...
export const pulsarDirectiveFormatter: Unstable_DirectiveFormatter = {
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(HERMES_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as PULSARRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: PULSARRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing PULSAR directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => pulsarDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.pulsarDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.pulsarDesktop?.readFileDataUrl(id)
