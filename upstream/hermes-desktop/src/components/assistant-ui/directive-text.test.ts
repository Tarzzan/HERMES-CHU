...
import { formatRefValue, pulsarDirectiveFormatter } from './directive-text'
...
describe('pulsarDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = pulsarDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = pulsarDirectiveFormatter.parse('@file:src/main.tsx the entry point')
