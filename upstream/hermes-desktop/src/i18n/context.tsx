...
import { getPULSARConfigRecord, type PULSARConfigRecord, savePULSARConfig } from '@/pulsar'
...
export interface I18nConfigClient {
  getConfig: () => Promise<PULSARConfigRecord>
  saveConfig: (config: PULSARConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.pulsarDesktop?.api) {
      return Promise.resolve({})
    }

    return getPULSARConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.pulsarDesktop?.api) {
    ...
    return savePULSARConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: PULSARConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: PULSARConfigRecord, locale: Locale): PULSARConfigRecord {
