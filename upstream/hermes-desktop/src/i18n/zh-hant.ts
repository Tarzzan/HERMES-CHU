...
export const zhHant = defineLocale({
  ...
  boot: {
    ready: 'PULSAR Desktop 已就緒',
    desktopBootFailedWithMessage: message => `桌面啟動失敗：${message}`,
    steps: {
      connectingGateway: '正在連線桌面閘道',
      loadingSettings: '正在載入 PULSAR 設定',
      loadingSessions: '正在載入最近工作階段',
      startingDesktopConnection: '正在啟動桌面連線',
      startingPULSARDesktop: '正在啟動 PULSAR Desktop…'
    },
    errors: {
      backgroundExited: 'PULSAR 背景程序已結束。',
      backgroundExitedDuringStartup: 'PULSAR 背景程序在啟動期間結束。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '您的 PULSAR 後端早於目前的桌面版本，可能無法正常運作。請更新以保持一致。',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面後端拒絕了該請求 (405 Method Not Allowed)。請嘗試重新啟動 PULSAR Desktop。',
  ...
  settings: {
    ...
    resetConfirm: '要將所有設定恢復為 PULSAR 預設值嗎？',
    exportFailed: '匯出失敗',
    resetFailed: '重設失敗',
    nav: {
      providers: '提供方',
    ...
    appearance: {
      ...
      colorMode: '色彩模式',
      colorModeDesc: '選擇固定模式，或讓 PULSAR 跟隨系統設定。',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: 'PULSAR 需要本機時間上下文時使用。留空則使用系統時區。',
      agent: {
        imageInputMode: '控制圖片附件如何傳送給模型。',
        maxTurns: 'PULSAR 停止一次執行前的工具呼叫輪次上限。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'PULSAR 單次檔案讀取可讀取的最大字元數。',
      approvals: {
        mode: 'PULSAR 如何處理需要明確批准的指令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'PULSAR 從應用程式內更新自身時，保留本機原始碼變更（stash）或丟棄（discard）。終端機更新一律會詢問。'
      }
    }),
    about: {
      ...
      automaticUpdatesDesc: 'PULSAR 會在背景自動檢查更新，並在有可用更新時通知你。',
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'PULSAR Desktop 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 PULSAR 後端，請使用遠端閘道。在下方按設定檔指定各自的遠端主機。',
      appliesTo: '套用至',
      allProfiles: '全部設定檔',
      ...
      localTitle: '本機閘道',
      localDesc: '在 localhost 啟動私有 PULSAR 後端。這是預設方式，可離線使用。',
      remoteTitle: '遠端閘道',
      remoteDesc:
        '將此桌面殼層連線至遠端 PULSAR 後端。託管閘道使用 OAuth 或帳號密碼；自託管閘道也可使用工作階段 Token。',
      remoteUrlTitle: '遠端 URL',
      remoteUrlDesc: '遠端儀表板後端的基礎 URL。支援路徑前綴，例如 /pulsar。',
      ...
      savedTitle: '閘道設定已儲存',
      restartingMessage: 'PULSAR Desktop 將使用已儲存的設定重新連線。',
      savedMessage: '已儲存，下次重新啟動後生效。',
      connectedTo: (baseUrl, version) => `已連線至 ${baseUrl}${version ? ` · PULSAR ${version}` : ''}`,
    ...
    providers: {
      ...
      intro: '使用訂閱登入，無需複製 API 金鑰。PULSAR 會在應用程式中為您完成瀏覽器登入。',
      connected: '已連線',
      collapse: '收合',
  ...
  commandCenter: {
    ...
    nav: {
      ...
      settings: { title: '設定', detail: '設定 PULSAR 桌面端' },
    ...
    pulsarActiveSessions: (version, count) => `PULSAR ${version} · 活躍工作階段 ${count}`,
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'Bot 使用者 ID', placeholder: '@pulsar:example.org' },
  ...
  profiles: {
    ...
    deleting: '刪除中…',
    createDesc: '設定檔是獨立的 PULSAR 環境：各自擁有獨立的設定、技能和 SOUL.md。',
    nameLabel: '名稱',
  ...
  cron: {
    ...
    newCron: '新排程工作',
    emptyDescNew:
      '按 cron 表達式排程一個提示詞。PULSAR 會執行它，並將結果傳送至您選擇的目的地。',
  ...
  composer: {
    ...
    placeholderStarting: '正在啟動 PULSAR...',
    placeholderReconnecting: '正在重新連線至 PULSAR…',
    ...
    attachUrlDesc: 'PULSAR 將擷取該頁面並作為此回合的脈絡。',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 PULSAR 無法在應用程式內自行更新。',
    connectionRetry: '請檢查網路連線後重試。',
    latestBody: '您正在執行最新版本。',
    latestBodyBackend: '後端正在執行最新版本。',
    allSetTitle: '已是最新版本',
    availableTitle: '有可用更新',
    availableBody: '新版 PULSAR 已可安裝。',
    availableTitleBackend: '後端有可用更新',
    availableBodyBackend: '已連接的 PULSAR 後端有新版本可安裝。',
    availableBodyNoChangelog: '已有新版本可用。此安裝方式無法顯示更新日誌。',
    updateNow: '立即更新',
    maybeLater: '稍後再說',
    moreChanges: count => `另有 ${count} 項變更。`,
    manualTitle: '從終端機更新',
    manualBody: '您是從命令列安裝的 PULSAR，因此更新也需要在那裡執行。請將此指令貼到終端機：',
    manualPickedUp: '下次啟動 PULSAR 時會使用新版本。',
    copy: '複製',
    copied: '已複製',
    done: '完成',
    applyingBody: 'PULSAR 更新程式會在自己的視窗中接管，並在完成後重新開啟 PULSAR。',
    applyingBodyBackend: '遠端後端正在套用更新並將重新啟動。恢復後 PULSAR 會自動重新連線。',
    applyingClose: 'PULSAR 將關閉以套用更新。',
    errorTitle: '更新未完成',
  ...
  install: {
    ...
    failedTitle: '安裝失敗',
    settingUpTitle: '正在設定 PULSAR Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某個安裝步驟失敗。在 Windows 上，如果另一個 PULSAR CLI 或桌面執行個體正在執行，可能會出現這種情況。請停止正在執行的 PULSAR 執行個體後重試。可查看下方的詳細資訊或 desktop 記錄中的完整記錄。',
    activeDesc:
      '這是一次性設定。PULSAR 安裝程式正在下載相依套件並設定您的電腦。之後啟動會略過此步驟。',
  ...
  onboarding: {
    headerTitle: '開始設定 PULSAR Agent',
    headerDesc: '連線模型提供方即可開始聊天。大多數選項只需一次點擊。',
    preparingInstall: 'PULSAR 正在完成安裝。首次執行通常不到一分鐘。',
    starting: '正在啟動 PULSAR…',
    ...
    recommended: '建議',
    connected: '已連線',
    featuredPitch: '一個訂閱，300+ 前沿模型 — 執行 PULSAR 的建議方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自託管',
        description: '將 PULSAR 指向本機或自託管的 OpenAI 相容端點（vLLM、llama.cpp、Ollama 等）。'
      }
    },
    backToSignIn: '返回登入',
    getKey: '取得金鑰',
    ...
    flowSubtitles: {
      ...
      device_code: '在瀏覽器中開啟驗證頁面 — PULSAR 會自動連線',
      loopback: '開啟瀏覽器登入 — PULSAR 會自動連線',
    ...
    authorizeThere: '請在那裡授權 PULSAR。',
    copyAuthCode: '複製授權碼並貼到下方。',
    pasteAuthCode: '貼上授權碼',
    reopenAuthPage: '重新開啟授權頁面',
    autoBrowser: provider =>
      `已在瀏覽器中開啟 ${provider}。請在那裡授權 PULSAR，連線會自動完成，無需複製或貼上。`,
  ...
  preview: {
    ...
    largeTitle: '此檔案較大',
    largeBody: (label, size) => `${label} 大小為 ${size}。PULSAR 只會顯示前 512 KB。`,
    previewAnyway: '仍然預覽',
    ...
    web: {
      ...
      failedToLoad: '預覽載入失敗',
      tryAgain: '重試',
      restarting: 'PULSAR 正在重新啟動...',
      askRestart: '請 PULSAR 重新啟動伺服器',
      lookingRestart: taskId => `PULSAR 正在尋找要重新啟動的預覽伺服器 (${taskId})`,
      restartingTitle: '正在重新啟動預覽伺服器',
      restartingMessage: 'PULSAR 正在背景執行。可在預覽主控台查看進度。',
  ...
  assistant: {
    ...
    approval: {
      gatewayDisconnected: 'PULSAR 閘道未連線',
      sendFailed: '無法傳送核准回應',
      run: '執行',
      moreOptions: '更多核准選項',
      ...
      alwaysDescription: pattern =>
        `這會將「${pattern}」模式加入永久允許清單（~/.pulsar/config.yaml）。PULSAR 對類似指令將不再詢問，包括目前工作階段和未來工作階段。`,
      alwaysAllow: '一律允許'
    },
    clarify: {
      notReady: '澄清請求尚未就緒',
  ...
  prompts: {
    ...
    sudoTitle: '管理員密碼',
    sudoDesc: 'PULSAR 需要您的 sudo 密碼來執行特權指令。它只會傳送給您的本機代理。',
    sudoPlaceholder: 'sudo 密碼',
    secretTitle: '需要密鑰',
    secretDesc: 'PULSAR 需要一個憑證才能繼續。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重新啟動 PULSAR Desktop 後可使用儲存圖片。',
    restartToSaveImages: '重新啟動 PULSAR Desktop 以儲存圖片',
