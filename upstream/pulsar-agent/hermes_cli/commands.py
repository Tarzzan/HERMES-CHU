...
COMMAND_REGISTRY: list[CommandDef] = [
    # Session
    CommandDef("start", "Accuser réception des pings de démarrage sans répondre", "Session",
               gateway_only=True),
    CommandDef("new", "Démarrer une nouvelle session (nouvel ID + historique vide)", "Session",
               aliases=("reset",), args_hint="[name]"),
    CommandDef("topic", "Activer ou inspecter les sessions par sujet Telegram", "Session",
               gateway_only=True, args_hint="[off|help|session-id]"),
    CommandDef("clear", "Effacer l'écran et démarrer une nouvelle session", "Session",
               cli_only=True),
    CommandDef("redraw", "Forcer un rafraîchissement complet de l'interface (récupère les décalages)", "Session",
               cli_only=True),
    CommandDef("history", "Afficher l'historique de la conversation", "Session",
               cli_only=True),
    CommandDef("save", "Sauvegarder la conversation en cours", "Session",
               cli_only=True),
    CommandDef("retry", "Réessayer le dernier message (renvoyer à l'agent)", "Session"),
    CommandDef("undo", "Reculer de N tours et relancer (défaut : 1)", "Session",
               args_hint="[N]"),
    CommandDef("title", "Définir un titre pour la session en cours", "Session",
               args_hint="[name]"),
    CommandDef("handoff", "Transférer la session vers une plateforme de messagerie (Telegram, Discord…)", "Session",
               args_hint="<platform>", cli_only=True),
    CommandDef("branch", "Créer une branche de la session (explorer une autre piste)", "Session",
               aliases=("fork",), args_hint="[name]"),
    CommandDef("compress", "Compresser le contexte de conversation (option 'here [N]' pour garder N tours récents)", "Session",
               args_hint="[here [N] | focus topic]"),
    CommandDef("rollback", "Lister ou restaurer des points de contrôle du système de fichiers", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Créer ou restaurer des instantanés de l'état PULSAR", "Session",
               cli_only=True, aliases=("snap",), args_hint="[create|restore <id>|prune]"),
    CommandDef("stop", "Arrêter tous les processus en arrière-plan", "Session"),
    CommandDef("approve", "Approuver une commande dangereuse en attente", "Session",
               gateway_only=True, args_hint="[session|always]"),
    CommandDef("deny", "Refuser une commande dangereuse en attente", "Session",
               gateway_only=True),
    CommandDef("background", "Exécuter un prompt en arrière-plan", "Session",
               aliases=("bg", "btw"), args_hint="<prompt>"),
    CommandDef("agents", "Afficher les agents actifs et les tâches en cours", "Session",
               aliases=("tasks",)),
    CommandDef("queue", "Mettre un prompt en file d'attente pour le prochain tour (sans interrompre)", "Session",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "Injecter un message après le prochain appel d'outil sans interrompre", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Définir un objectif persistant que PULSAR poursuit jusqu'à réalisation", "Session",
               args_hint="[text | pause | resume | clear | status]"),
    CommandDef("subgoal", "Ajouter ou gérer des critères supplémentaires sur l'objectif actif", "Session",
               args_hint="[text | remove N | clear]"),
    CommandDef("status", "Afficher les informations de la session", "Session"),
    CommandDef("whoami", "Afficher votre niveau d'accès aux commandes (admin / utilisateur)", "Info"),
    CommandDef("profile", "Afficher le profil actif et le répertoire de base", "Info"),
    CommandDef("sethome", "Définir ce chat comme canal principal", "Session",
               gateway_only=True, aliases=("set-home",)),
    CommandDef("resume", "Reprendre une session précédemment nommée", "Session",
               args_hint="[name]"),
    ...
    CommandDef("config", "Afficher la configuration actuelle", "Configuration",
               cli_only=True),
    CommandDef("model", "Changer de modèle pour cette session", "Configuration",
               args_hint="[model] [--provider name] [--global] [--refresh]"),
    CommandDef("codex-runtime", "Basculer le runtime codex app-server pour les modèles OpenAI/Codex",
               "Configuration", aliases=("codex_runtime",),
               args_hint="[auto|codex_app_server]"),
    CommandDef("gquota", "Afficher l'utilisation du quota Google Gemini Code Assist", "Info",
               cli_only=True),

    CommandDef("personality", "Définir une personnalité prédéfinie", "Configuration",
               args_hint="[name]"),
    CommandDef("statusbar", "Afficher/masquer la barre de statut contexte/modèle", "Configuration",
               cli_only=True, aliases=("sb",)),
    CommandDef("verbose", "Cycler l'affichage de progression des outils : off → nouveau → tout → verbeux",
               "Configuration", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("footer", "Afficher/masquer le pied de page métadonnées sur les réponses finales",
               "Configuration", args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("yolo", "Activer/désactiver le mode YOLO (ignorer les confirmations de commandes dangereuses)",
               "Configuration"),
    CommandDef("reasoning", "Gérer l'effort de raisonnement et son affichage", "Configuration",
               args_hint="[level|show|hide]",
               subcommands=("none", "minimal", "low", "medium", "high", "xhigh", "show", "hide", "on", "off")),
    CommandDef("fast", "Basculer le mode rapide — Traitement prioritaire OpenAI / Mode rapide Anthropic", "Configuration",
               args_hint="[normal|fast|status]",
               subcommands=("normal", "fast", "status", "on", "off")),
    CommandDef("skin", "Afficher ou changer le thème visuel du terminal", "Configuration",
               cli_only=True, args_hint="[name]"),
    CommandDef("indicator", "Choisir le style d'indicateur d'activité du terminal", "Configuration",
               cli_only=True, args_hint="[kaomoji|emoji|unicode|ascii]",
               subcommands=("kaomoji", "emoji", "unicode", "ascii")),
    CommandDef("voice", "Activer/désactiver le mode vocal", "Configuration",
               args_hint="[on|off|tts|status]", subcommands=("on", "off", "tts", "status")),
    CommandDef("busy", "Contrôler ce que fait Entrée pendant que PULSAR travaille", "Configuration",
               cli_only=True, args_hint="[queue|steer|interrupt|status]",
               subcommands=("queue", "steer", "interrupt", "status")),

    # Tools & Skills
    CommandDef("tools", "Gérer les outils : /tools [list|disable|enable] [nom…]", "Tools & Skills",
               args_hint="[list|disable|enable] [name...]", cli_only=True),
    CommandDef("toolsets", "Lister les ensembles d'outils disponibles", "Tools & Skills",
               cli_only=True),
    CommandDef("skills", "Rechercher, installer, inspecter ou gérer les compétences",
               "Tools & Skills", cli_only=True,
               subcommands=("search", "browse", "inspect", "install", "audit",
                            "pending", "approve", "reject", "diff", "approval")),
    CommandDef("memory", "Vérifier les écritures mémoire en attente / basculer la validation",
               "Tools & Skills",
               args_hint="[pending|approve|reject|approval] [id|on|off]",
               subcommands=("pending", "approve", "reject", "approval")),
    CommandDef("bundles", "Lister les paquets de compétences (alias /<nom> pour plusieurs compétences)",
               "Tools & Skills"),
    CommandDef("cron", "Gérer les tâches planifiées", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("curator", "Maintenance des compétences en arrière-plan (statut, exécuter, épingler, archiver)",
               "Tools & Skills", args_hint="[subcommand]",
               subcommands=("status", "run", "pause", "resume", "pin", "unpin", "restore", "list-archived")),
    CommandDef("kanban", "Tableau de collaboration multi-profil (tâches, liens, commentaires)",
               "Tools & Skills", args_hint="[subcommand]",
    ...
    CommandDef("reload", "Recharger les variables .env dans la session en cours", "Tools & Skills",
               cli_only=True),
    CommandDef("reload-mcp", "Recharger les serveurs MCP depuis la configuration", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Rescanner ~/.hermes/skills/ pour les compétences installées ou supprimées",
               "Tools & Skills", aliases=("reload_skills",)),
    CommandDef("browser", "Connecter les outils navigateur à votre Chromium via CDP", "Tools & Skills",
               cli_only=True, args_hint="[connect|disconnect|status]",
               subcommands=("connect", "disconnect", "status")),
    CommandDef("plugins", "Lister les plugins installés et leur statut",
               "Tools & Skills", cli_only=True),

    # Info
    CommandDef("commands", "Parcourir toutes les commandes et compétences (paginé)", "Info",
               gateway_only=True, args_hint="[page]"),
    CommandDef("help", "Afficher les commandes disponibles", "Info"),
    CommandDef("restart", "Redémarrer la passerelle proprement après avoir drainé les exécutions actives", "Session",
               gateway_only=True),
    CommandDef("usage", "Afficher l'utilisation des tokens et les limites de débit de la session", "Info"),
    CommandDef("insights", "Afficher les statistiques et analyses d'utilisation", "Info",
               args_hint="[days]"),
    CommandDef("platforms", "Afficher le statut des plateformes de messagerie", "Info",
               cli_only=True, aliases=("gateway",)),
    CommandDef("platform", "Mettre en pause, reprendre ou lister une plateforme en échec", "Info",
               gateway_only=True, args_hint="<pause|resume|list> [name]"),
    CommandDef("copy", "Copier la dernière réponse de l'assistant dans le presse-papiers", "Info",
               cli_only=True, args_hint="[number]"),
    CommandDef("paste", "Joindre une image depuis le presse-papiers", "Info",
               cli_only=True),
    CommandDef("image", "Joindre un fichier image local pour votre prochain prompt", "Info",
               cli_only=True, args_hint="<path>"),
    CommandDef("update", "Mettre à jour PULSAR vers la dernière version", "Info"),
    CommandDef("version", "Afficher la version de PULSAR", "Info", aliases=("v",)),
    CommandDef("debug", "Envoyer un rapport de débogage (infos système + journaux) et obtenir des liens partageables", "Info"),

    # Exit
    CommandDef("quit", "Quitter le CLI (option --delete pour supprimer aussi l'historique)", "Exit",
