...
export const ja: Translations = {
  ...
  status: {
    ...
    updatingHermes: "PULSAR を更新しています…",
  ...
  pluginsPage: {
    ...
    headline:
      "PULSAR プラグインを発見、インストール、有効化、更新します (`hermes plugins` 相当)。",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "実際のセッション履歴から獲得できる PULSAR のコレクタブル バッジです。既知の未達成の実績は「Discovered」として表示され、Secret 実績は最初の該当する挙動が検出されるまで非表示のままです。",
      scan_subtitle:
        "PULSAR のセッション履歴をスキャンしています。履歴が大きい場合、初回スキャンには 5～10 秒かかることがあります。",
    },
    actions: {
      rescan: "再スキャン",
    },
    stats: {
      ...
      latest: "最新",
      latest_hint_empty: "PULSAR をもっと使ってみてください",
      none_yet: "まだありません",
    },
    state: {
      unlocked: "解除済み",
    ...
    guide: {
      ...
      secret_body:
        "シークレットはトリガー条件を隠しています。PULSAR が関連するシグナルを検出すると、カードは「Discovered」になり、要件が表示されます。",
      scan_status_header: "スキャン状況",
      scan_status_body:
        "PULSAR はローカル履歴を一度スキャンし、その後カードが自動的に表示されます。数秒かかってもスタックしているわけではありません。",
