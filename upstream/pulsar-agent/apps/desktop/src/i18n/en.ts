...
export const en: Translations = {
  ...
  boot: {
    ready: 'PULSAR Desktop is ready',
    desktopBootFailedWithMessage: message => `Desktop boot failed: ${message}`,
    steps: {
      connectingGateway: 'Connecting live desktop gateway',
      loadingSettings: 'Loading PULSAR settings',
      loadingSessions: 'Loading recent sessions',
      startingDesktopConnection: 'Starting desktop connection',
      startingHermesDesktop: 'Starting PULSAR Desktop…'
    },
    errors: {
      backgroundExited: 'PULSAR background process exited.',
      backgroundExitedDuringStartup: 'PULSAR background process exited during startup.',
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Your PULSAR backend is older than this desktop build and may not work correctly. Update to align them.',
    ...
    errors: {
      ...
      methodNotAllowed:
        'The desktop backend rejected that request (405 Method Not Allowed). Try restarting PULSAR Desktop.',
  ...
  settings: {
    ...
    resetConfirm: 'Reset all settings to PULSAR defaults?',
    ...
    appearance: {
      ...
      colorModeDesc: 'Pick a fixed mode or let PULSAR follow your system setting.',
    ...
    about: {
      ...
      automaticUpdatesDesc:
        'PULSAR checks for updates automatically in the background and lets you know when one is ready.',
    ...
    config: {
      ...
      loading: 'Loading PULSAR configuration...',
    ...
    gateway: {
      ...
      intro:
        'PULSAR Desktop starts its own local gateway by default. Use a remote gateway when you want this app to control an already-running PULSAR backend on another machine or behind a trusted proxy. Pick a profile below to give it its own remote host.',
      ...
      localDesc: 'Start a private PULSAR backend on localhost. This is the default and works offline.',
      remoteTitle: 'Remote gateway',
      remoteDesc:
        'Connect this desktop shell to a remote PULSAR backend. Hosted gateways use OAuth or a username and password; self-hosted ones may use a session token.',
      ...
      restartingMessage: 'PULSAR Desktop will reconnect using the saved settings.',
      savedMessage: 'Saved for the next restart.',
      connectedTo: (baseUrl, version) => `Connected to ${baseUrl}${version ? ` · PULSAR ${version}` : ''}`,
    ...
    providers: {
      ...
      intro:
        'Sign in with a subscription — no API key to copy. PULSAR runs the browser sign-in for you, right here in the app.',
      connected: 'Connected',
      collapse: 'Collapse',
  ...
  commandCenter: {
    ...
    nav: {
      ...
      settings: { title: 'Settings', detail: 'Configure PULSAR desktop' },
    ...
    hermesActiveSessions: (version, count) => `PULSAR ${version} · Active sessions ${count}`,
  ...
  profiles: {
    ...
    deleting: 'Deleting...',
    createDesc: 'Profiles are independent PULSAR environments: separate config, skills, and SOUL.md.',
    nameLabel: 'Name',
  ...
  cron: {
    ...
    newCron: 'New cron',
    emptyDescNew:
      'Schedule a prompt to run on a cron expression. PULSAR will run it and deliver results to the destination you pick.',
  ...
  composer: {
    ...
    placeholderStarting: 'Starting PULSAR...',
    placeholderReconnecting: 'Reconnecting to PULSAR…',
    ...
    attachUrlDesc: 'PULSAR will fetch the page and include it as context for this turn.',
  ...
  updates: {
    ...
    unsupportedMessage: 'This version of PULSAR can’t update itself from inside the app.',
    ...
    manualBody: 'You installed PULSAR from the command line, so updates run there too. Paste this into your terminal:',
    manualPickedUp: 'PULSAR will pick up the new version next time you launch it.',
    copy: 'Copy',
    copied: 'Copied',
    done: 'Done',
    applyingBody: 'The PULSAR updater will take over in its own window and reopen PULSAR when it’s done.',
    applyingBodyBackend: 'The remote backend is applying the update and will restart. PULSAR reconnects automatically when it’s back.',
    applyingClose: 'PULSAR will close to apply the update.',
  ...
  install: {
    ...
    oneTimeTitle: 'PULSAR needs a one-time install',
    ...
    settingUpTitle: 'Setting up PULSAR',
    finishingTitle: 'Finishing up',
    failedDesc:
      'One of the install steps failed. On Windows, this can happen if another PULSAR CLI or desktop instance is running. Stop any running PULSAR instances, then retry. Check the details below or the desktop log for the full transcript.',
    activeDesc:
      'This is a one-time setup. The PULSAR installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.',
  ...
  onboarding: {
    headerTitle: "Let's get you setup with PULSAR",
    headerDesc: 'Connect a model provider to start chatting. Most options take one click.',
    preparingInstall: 'PULSAR is finishing install. This usually takes under a minute on first run.',
    starting: 'Starting PULSAR…',
    ...
    recommended: 'Recommended',
    connected: 'Connected',
    featuredPitch: 'One subscription, 300+ frontier models — the recommended way to run PULSAR',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'self-hosted',
        description: 'Point PULSAR at a local or self-hosted OpenAI-compatible endpoint (vLLM, llama.cpp, Ollama, etc).'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'Opens a verification page in your browser — PULSAR connects automatically',
      loopback: 'Opens your browser to sign in — PULSAR connects automatically',
    ...
    authorizeThere: 'Authorize PULSAR there.',
    ...
    autoBrowser: provider =>
      `We opened ${provider} in your browser. Authorize PULSAR there and you'll be connected automatically — nothing to copy or paste.`,
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'PULSAR inference gateway status',
      agents: 'Agents',
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} is ${size}. PULSAR will only show the first 512 KB.`,
    ...
    web: {
      ...
      tryAgain: 'Try again',
      restarting: 'PULSAR is restarting...',
      askRestart: 'Ask PULSAR to restart the server',
      lookingRestart: taskId => `PULSAR is looking for a preview server to restart (${taskId})`,
      restartingTitle: 'Restarting preview server',
      restartingMessage: 'PULSAR is working in the background. Watch the preview console for progress.',
      ...
      finishedRestarting: message => `PULSAR finished restarting the preview server${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'PULSAR could not restart the server.',
      stillWorking:
        'PULSAR is still working, but no restart result has arrived yet. The server command may be running in the foreground.',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'PULSAR is loading a response',
      thinking: 'Thinking',
    ...
    approval: {
      gatewayDisconnected: 'PULSAR gateway is not connected',
      ...
      alwaysDescription: pattern =>
        `This adds the “${pattern}” pattern to your permanent allowlist (~/.hermes/config.yaml). PULSAR won’t ask again for commands like this — in this session or any future one.`,
  ...
  prompts: {
    ...
    sudoDesc: 'PULSAR needs your sudo password to run a privileged command. It is sent only to your local agent.',
    sudoPlaceholder: 'sudo password',
    secretTitle: 'Secret required',
    secretDesc: 'PULSAR needs a credential to continue.',
  ...
  desktop: {
    ...
    restartToUseSaveImage: 'Restart PULSAR Desktop to use Save Image.',
    restartToSaveImages: 'Restart PULSAR Desktop to save images',
